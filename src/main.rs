...
async fn main() {
    ...
    loop {
        ...
        if get_time() > 0. {
            car.update(get_frame_time() as f64);
        }

        if (car.enable){
            forceplt.update([car.get_last_state().unwrap().v, car.a]
                .to_vec(), car.get_last_state().unwrap().th);
            forceplt1.update([car.get_last_state().unwrap().x].to_vec(), car.get_last_state().unwrap().th);
        }
        ...
        car.display(back_color, WHITE, 0.006, 6. * grid, 3. * grid);
        draw_speedometer(
            &format!(
                "Velocidade ({}) {:.2}",
                if car.state.v.is_sign_negative() {
                    "-"
                } else {
                    "+"
                },
        ...
        draw_ui(w_init, grid, &mut car, &mut forceplt, &mut forceplt1, &mut report);
